...
test.describe("首页", () => {
	...
	test("分类导航存在并可切换", async ({ page, isMobile }) => {
		...
		// 桌面端：Sidebar 的 nav[aria-label="导航分类"]（md:block）
		// 移动端：MobileNav 的 nav[aria-label="移动端导航分类"]（dynamic ssr:false，需等待 hydrate）
		const categoryNav = page.locator(
			isMobile
				? 'nav[aria-label="移动端导航分类"]'
				: 'nav[aria-label="导航分类"]'
		);
		...
		if (await categoryTab.isVisible()) {
			...
			// 桌面端：面包屑出现（Breadcrumb nav text="首页 / AI"）
			// 移动端：无面包屑，但分类标签变为 active（aria-selected="true"）
			if (!isMobile) {
...
test.describe("工具详情页", () => {
	...
	test("Figma 工具详情页正确加载", async ({ page }) => {
		...
		// 访问官网按钮（多个 section 可能有多个，取第一个可见的）
...
test.describe.serial("ToolQuickView 预览弹窗", () => {
	...
	test.beforeEach(async ({ page }) => {
		// Retry page load if data not available (handles transient Supabase connection throttling).
		// Each retry triggers a fresh SSR request which may succeed after prior connections release.
		await expect(async () => {
			await page.goto("/", { waitUntil: "domcontentloaded" });
			await expect(page.locator('[data-nav-hydrated="true"]')).toBeAttached({ timeout: 5000 });
			await expect(page.locator(".card-hover").first()).toBeVisible({ timeout: 5000 });
		}).toPass({ timeout: 120_000, intervals: [1_000] });

		await expect(page.locator('button[aria-label^="预览"]').first()).toBeVisible({ timeout: 30000 });
	});
