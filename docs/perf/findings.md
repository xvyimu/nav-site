...
- **✅ 已修复（3）**：H1 + H2/H5 + H10 —— H1 pangu 子树限定（架构更优，CI 量化收益不显著）；H2/H5 移除 `layout` prop（CI TBT 85→36ms）；H10 从初始加载路径彻底移除 motion/react（首屏 JS 469.8→429.6 KB, -8.5%）。
...
**接手者下一步**：(1) Navigation.tsx 拆分（将分类导航抽为独立 client 组件，主布局 RSC 化，预计再减 motion ~40KB）← ⚠️ H10 已从首屏移除 motion（仅剩动态 chunk），此项收益重估：Navigation.tsx 拆分的主要目标转为架构优化； (2) H7 方案 3（换 Sentry entry，破坏性）；(3) Sentry Web Vitals P75 数据积累后评估真实 INP/LCP。
...
## H10: 初始加载路径残留 motion/react（ShortcutPanel/login/SubmitForm）（2026-06-30）

### 假设陈述

H6 的 RSC 边界收缩（移除 CategorySection / DualTrackSection 的 `"use client"` + `motion.section`）
未使首屏 JS 减少（+0.3KB，噪声内），根因是 Navigation.tsx 作为根 client boundary 仍将 motion/react
打入首屏。但进一步分析 client.html chunk 树发现更有直接的问题：**即使不考虑 Navigation.tsx，
初始加载路径中仍有 3 处独立的 motion 引用**，分布在 ShortcutPanel（`AnimatePresence`）、
登录页面（`motion.div`）、和提交表单（`motion.div` / `motion.form`）中。

这三个组件属于不同的路由（layout/shortcut-panel、/login、/submit），但 webpack
的代码分割会将共享依赖（如 `AnimatePresence`）hoist 到父 chunk 中，即使子 chunk
是动态导入 —— 因为 ShortcutPanel 通过 layout 层被所有页面引用，其 `AnimatePresence`
import 被 webpack 提升到了 layout 的初始 chunk（chunk 1915 + chunk 2554）。

### 验证方法

1. 用 `extract-bundle-stats.mjs`（已修复 H9 的 4 个 bug）解析 `pnpm analyze` 输出
2. 对每个初始 chunk（`isInitialByEntrypoint: true`），递归检查其子树是否含 motion-dom / framer-motion
3. 确认 chunk 归属路由器映射

### 验证结果

**✅ 已坐实，已修复（2026-06-30 bundle 分析 + 代码审查）**

通过 `client.html` 的 chunk 树发现两个初始 chunk 含 motion：

| chunk | 大小 | 初始归属 | motion 来源路径 |
|---|---|---|---|
| 1915 | 8.4 KB | app/layout | AnimatePresence（通过 ShortcutPanel 动态 import 被 hoist） |
| 2554 | 38.4 KB | app/layout + app/login/page + app/submit/page | motion-dom（ShortcutPanel motion.div + login motion.div + SubmitForm motion.div/motion.form） |

三个具体的 motion 引用：

1. **ShortcutPanel** (`components/ShortcutPanel.tsx`)：`AnimatePresence` + `motion.div` ×2
   - 通过 `dynamic(() => import("./ShortcutPanel"))` 在 layout 中懒加载
   - 但 `AnimatePresence` 作为共享依赖被 webpack 提升到 layout 的初始 chunk（chunk 1915）
2. **登录页面** (`app/login/page.tsx`)：`motion.div` ×1
   - 独立路由，但与 layout 共享 chunk（chunk 2554 同时是 app/layout 的初始 chunk）
3. **提交表单** (`app/submit/page.tsx` → `components/SubmitForm.tsx`)：`motion.div` + `motion.form` ×2
   - 同上，与 layout 共享 chunk

### 修复方案

**全部替换为 CSS 动画**，无例外。三个组件零交互依赖 motion，纯入场动画。

#### ShortcutPanel 改动

- 删除 `import { motion, AnimatePresence } from "motion/react"`
- 替换 `<AnimatePresence>` → `{open && (...)}`
- 替换 backdrop `<motion.div>` → `<div className="... animate-fade-in">`
- 替换 panel `<motion.div>` → `<div className="... animate-fade-in-up">`
- 丢失：exit 动画（fade out / scale out）——实用面板，瞬间消失可接受

#### login/page.tsx 改动

- 删除 `import { motion } from "motion/react"`
- 替换 `<motion.div>` → `<div className="w-full max-w-sm animate-fade-in-up">`
- 丢失：无（motion 只有入场动画、CSS 版本等价）

#### SubmitForm.tsx 改动

- 删除 `import { motion } from "motion/react"`
- 替换 success state `<motion.div>` → `<div className="... animate-fade-in-up">`
- 替换 form `<motion.form>` → `<form className="... animate-fade-in-up">`
- 丢失：无（motion 只有入场动画、CSS 版本等价）

**注意**：`lib/animations.ts` 仍保留 `import { type Variants } from "motion/react"` ——
这是 type-only import，SWC 编译期完全擦除，无运行时开销。
该文件仅由 `ModelRanking.tsx`（`dynamic(() => import(...), { ssr: false })`）引用。

### before/after 数据

**Bundle 体积（gzip，`pnpm analyze`）**：

| 指标 | before（H9 基线） | after（H10） | delta |
|---|---|---|---|
| **client 首屏 JS** | **469.8 KB** | **429.6 KB** | **-40.2 KB（-8.5%）** |
| client total JS | 483.5 KB | 478.5 KB | -5.0 KB |
| chunk 数量 | 63 | 62 | -1 |
| motion-dom（总） | 28.5 KB | 28.2 KB | -0.3 KB |
| framer-motion（总） | 11.1 KB | 8.7 KB | -2.4 KB |

**首屏 JS 的包级分解**（after，gzip）：

| 包 | 大小 | 来源 |
|---|---|---|
| next（框架运行时） | 266.8 KB | ❌ 框架地板 |
| @sentry/core | 68.2 KB | ⚠️ 见 H7 |
| react-dom | 55.1 KB | ❌ 框架地板 |
| (app code) | 50.8 KB | ⚠️ 应用代码 |
| @sentry/* 其余 | 35.5 KB | ⚠️ 见 H7 |
| lucide-react | 17.3 KB | ⚠️ 可审查 |
| sonner | 9.0 KB | ⚠️ 见 H9 |
| motion-dom + framer-motion | **0 KB 在首屏** | ✅ 仅动态 chunk |
...
这是 Phase 2 中**单项收益最大的改动**（-40.2 KB，-8.5%），所有 3 处 motion 引用均被 CSS 动画替代。
motion/react 已完全从初始加载路径中移除，仅在动态 chunk（ModelRanking）中保留。

**丢失用户体验**：
- ShortcutPanel 的 exit 动画（打开后按 Esc 消失时瞬间消失而非淡出）
- 这三处原本是非常轻量的入场动画（opacity + translateY），CSS 版本等价且无 JS 开销

**后续剩余可寻址空间**（达到 250KB 目标仍需 -179.6 KB）：
...
（Navigation.tsx 拆分 → useLinksFilter 服务端化），不在纯 bundle 分析范围内。

### 涉及文件

| 文件 | 改动 |
|---|---|
| `app/login/page.tsx` | motion.div → CSS animate-fade-in-up |
| `components/ShortcutPanel.tsx` | AnimatePresence + motion.div ×2 → CSS animate-fade-in/-up |
| `components/SubmitForm.tsx` | motion.div + motion.form → CSS animate-fade-in-up |
