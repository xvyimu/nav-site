...
### P1（高优先级）
- [ ] 评分/稳定性标签（调用成功率、响应速度标签）
- [ ] 卡片 hover 预览（悬浮时显示更多信息）
- [ ] 数据最后更新时间戳
- [ ] 链接点击计数（热度排名）
- [ ] 基础暗色模式（根据系统偏好自动切换）
- [ ] 缓存策略升级（`actions/cache@v4` 加速 CI）
...
1. **P1 第一阶段：** 稳定性标签 + 点击计数 + 最后更新时间
2. **P1 第二阶段：** 暗色模式（利用 next-themes 或 CSS 变量切换）
3. **P2：** 今日推荐大卡片 + 热门排行
4. **P3：** API 状态检测 + 更多装饰效果

---

> 文档版本 v1.0 · 2026-06-20 · 基于 5 个设计参考综合提炼
